...
def calcular_punto():
    try:
        ...
        y_val = f.subs(x_sym, x_val).evalf()
        resultado_punto.configure(text=f"Punto: ({x_val}, {y_val})")
    except Exception as e:
...
def crear_interfaz_grafica():
    ...
    boton_punto = ctk.CTkButton(ventana, text="Calcular punto", command=calcular_punto)
    boton_punto.grid(row=3, column=1, padx=(100, 50), pady=(10, 50), sticky="ew")
    resultado_punto = ctk.CTkLabel(ventana, text="Punto: ---")
    resultado_punto.grid(row=3, column=1, padx=(100, 50), pady=(50, 20), sticky="w")


    boton_Grafico = ctk.CTkButton(ventana, text="Mostrar Grafico", command=GraficaPLT)
    boton_Grafico.grid(row=4, column=1, padx=(100, 50), pady=(10,50), sticky="ew")
